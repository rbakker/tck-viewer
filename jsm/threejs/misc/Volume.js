...
class Volume {
	 ...
	 */
	extractPerpendicularPlane( axis, index ) {
		...
		switch ( axis ) {
			case 'x' :
				...
				firstSpacing = 1;
				secondSpacing = 1;
				IJKIndex = new Vector3( index, 0, 0 );

				planeMatrix.multiply( ( new Matrix4() ).makeRotationY( Math.PI / 2 ) );
				positionOffset = ( volume.dimensions[ 0 ] - 1 ) / 2;
				planeMatrix.setPosition( new Vector3( index - positionOffset, 0, 0 ) );
				break;
			case 'y' :
				axisInIJK.set( 0, 1, 0 );
				...
				firstSpacing = 1;
				secondSpacing = 1;
				IJKIndex = new Vector3( 0, index, 0 );

				planeMatrix.multiply( ( new Matrix4() ).makeRotationX( - Math.PI / 2 ) );
				positionOffset = ( volume.dimensions[ 1 ] - 1 ) / 2;
				planeMatrix.setPosition( new Vector3( 0, index - positionOffset, 0 ) );
				break;
			case 'z' :
				axisInIJK.set( 0, 0, 1 );
				firstDirection.set( 1, 0, 0 );
				...
				firstSpacing = 1;
				secondSpacing = 1;
				IJKIndex = new Vector3( 0, 0, index );

				positionOffset = ( volume.dimensions[ 2 ] - 1 ) / 2;
				planeMatrix.setPosition( new Vector3( 0, 0, index - positionOffset ) );
				break;
        ...
        */
		...
		const iDirection = [ firstDirection, secondDirection, axisInIJK ].find( function ( x ) {
			...
			return Math.abs( x.dot( base[ 0 ] ) ) > 0.7;

		} );
		const jDirection = [ firstDirection, secondDirection, axisInIJK ].find( function ( x ) {

			return Math.abs( x.dot( base[ 1 ] ) ) > 0.7;

		} );
		const kDirection = [ firstDirection, secondDirection, axisInIJK ].find( function ( x ) {

			return Math.abs( x.dot( base[ 2 ] ) ) > 0.7;

		} );*/


        let sliceAccess,iLength,jLength;
        /*if (axis == 'x') {
		    sliceAccess = ( i, j ) => volume.access( index, volume.yLength-1-j, volume.zLength - 1-i );
            iLength = volume.zlength;
            jLength = volume.xLength;
        }
        if (axis == 'y') {
		    sliceAccess = ( i, j ) => volume.access( i, index, j );
            iLength = volume.xlength;
            jLength = volume.zLength;
        }
        if (axis == 'z') {
		    sliceAccess = ( i, j ) => volume.access( i, volume.yLength-1-j, index );
            iLength = volume.xlength;
            jLength = volume.yLength;
        }*/
